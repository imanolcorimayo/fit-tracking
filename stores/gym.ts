...
import type { GymSession, WorkoutExercise } from '~/stores/types/gym.types'
...
export const useGymStore = defineStore('gym', () => {
  ...
  const createSession = async (sessionData: Omit<GymSession, 'id' | 'userUid' | 'createdAt' | 'updatedAt'>): Promise<string | null> => {
    ...
    try {
      ...
      const docRef = await addDoc(collection(db, 'gym_sessions'), newSession)
      
      // Add the session to local state
      const sessionWithId: GymSession = {
        ...newSession,
        id: docRef.id
      }
  ...
  const getSession = async (sessionId: string): Promise<GymSession | null> => {
    if (!db || !user.value) {
      error.value = 'Database or user not available'
      return null
    }
    ...
    try {
      const docRef = doc(db, 'gym_sessions', sessionId)
  ...
  const updateSession = async (sessionId: string, updates: Partial<Omit<GymSession, 'id' | 'userUid' | 'createdAt'>>): Promise<boolean> => {
    if (!db || !user.value) {
      error.value = 'Database or user not available'
      return false
    }
    ...
    try {
      ...
      const sessionRef = doc(db, 'gym_sessions', sessionId)
  ...
  const deleteSession = async (sessionId: string): Promise<boolean> => {
    ...
    try {
      ...
      await deleteDoc(doc(db, 'gym_sessions', sessionId))
